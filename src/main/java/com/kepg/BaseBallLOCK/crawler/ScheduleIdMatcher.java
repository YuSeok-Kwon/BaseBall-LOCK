...
import com.kepg.BaseBallLOCK.game.RecordService;
...
public class ScheduleIdMatcher {
    ...
    public void run() {
        ...
        for (int statizId = startId; statizId <= endId; statizId++) {
            ...
            try {
                ...
                String awayTeamName = headers.get(0).text().replace("타격기록", "").replace("(", "").replace(")", "").trim();
                String homeTeamName = headers.get(1).text().replace("타격기록", "").replace("(", "").replace(")", "").trim();
                ...
                for (Element section : doc.select("div.box_type_boared")) {
                    ...
                    String teamName = head.text().replace("투구기록", "").trim();
                    ...
                    for (Element row : table.select("tbody > tr:not(.total)")) {
                        ...
                        if (name.contains("승")) {
                        	decision = "W";
                        } else if (name.contains("패")) {
                        	decision = "L";
                        } else if (name.contains("세")) {
                        	decision = "SV";
                        } else if (name.contains("홀")) {
                        	decision = "H";
                        }
