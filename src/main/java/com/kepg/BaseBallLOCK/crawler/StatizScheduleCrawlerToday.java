...
public class StatizScheduleCrawlerToday {
    ...
    static {
        teamNameToId.put("KIA", 1);  teamNameToId.put("두산", 2);  teamNameToId.put("삼성", 3);  teamNameToId.put("SSG", 4);
        teamNameToId.put("LG", 5);   teamNameToId.put("한화", 6); teamNameToId.put("NC", 7);    teamNameToId.put("KT", 8);
        teamNameToId.put("롯데", 9); teamNameToId.put("키움", 10);
    ...
    public void crawlTodaySchedule() {
        ...
        String url = String.format("https://statiz.sporki.com/schedule/?year=%d&month=%d", year, month);
        String targetDateStr = today.format(DateTimeFormatter.ofPattern("yyyyMMdd"));
        Timestamp matchDate = Timestamp.valueOf(today.atStartOfDay());
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            for (Element td : tdElements) {
                ...
                if (dayElement == null || !dayElement.text().trim().equals(String.valueOf(day))) continue;
                ...
                for (Element game : gameItems) {
                    ...
                    System.out.printf("저장 완료: %s - %s(%s) vs %s(%s) [%s]\n",
                            targetDateStr, homeTeam, homeScore != null ? homeScore : "-",
